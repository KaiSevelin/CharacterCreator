...
export class SkillTreeChargenApp extends FormApplication {
    ...
    static _normalizeSetupTables(opts = {}) {
        return {
            startingTable: String(opts.startingTable ?? "").trim(),
            contactTables: {
    ...
    static async open(opts = {}) {
        const preflight = await this.validateEnvironment(opts);
        if (!preflight.ok) {
    ...
    static _validateChangeSchema(ch, tableName, rewardIdx, changeIdx) {
        ...
        if (type === "skill") {
            ...
            if (!targetKey) {
                throw new Error(
                    `Skill change requires "targetKey" (or legacy "skill") in "${tableName}" (rewards[${rewardIdx}].changes[${changeIdx}]).`
                );
            }
            if (ch.targetLevel != null) {
                SkillTreeChargenApp._requireFiniteNumber(
                    ch.targetLevel,
                    `Skill change "targetLevel" must be numeric in "${tableName}" (rewards[${rewardIdx}].changes[${changeIdx}]).`
                );
            }
            return;
        }
    ...
    static async validateCareerTableGraph({ startingTable }) {
        const report = {
            ok: false,
            startingTable,
            checkedAt: new Date().toISOString(),
    ...
    static async parseRollTableResult(result, tableName = "RollTable") {
        const item = await SkillTreeChargenApp._resolveItemFromRollResult(result);
        if (item) {
            return SkillTreeChargenApp._parseTemplateItemToChoiceData(item, tableName);
        }

        const raw = SkillTreeChargenApp._resultRawJSON(result);
        if (!raw) throw new Error(`Empty result in ${tableName}.`);
        return SkillTreeChargenApp._parseJSONResultText(raw, tableName);
    }
    ...
    static _parseJSONResultText(text, tableName = "RollTable") {
        ...
        SkillTreeChargenApp._validateParsedResultSchema(obj, tableName);
        ...
        const rewards = (Array.isArray(obj.rewards) ? obj.rewards : []).map((r, i) => {
            ...
            if (Array.isArray(rr.changes)) {
                for (const ch of rr.changes) {
                    if (!ch || typeof ch !== "object") continue;
                    if (ch.type === "skill" && !ch.targetKey && ch.skill) {
                        ch.targetKey = ch.skill;
                    }
                }
            }
        ...
        const effectTables = (Array.isArray(obj.effectTables) ? obj.effectTables : []).map((tbl, tableIdx) => {
            ...
            const rows = (Array.isArray(tt?.rows) ? tt.rows : []).map((row, rowIdx) => {
                ...
                if (change?.type === "skill" && !change.targetKey && change.skill) {
                    change.targetKey = change.skill;
                }
    ...
    _resolveRewardFromEffectTables(effectTables) {
        ...
        for (const tbl of tables) {
            const row = this._pickWeightedEffectRow(tbl?.rows ?? []);
            if (!row) continue;
    ...
    async _applyChanges(run, changes = []) {
        for (const ch of changes) {
            ...
            if (ch.type === "language") {
                await this._awardLanguage(run, ch);
                continue;
            }
            if (ch.type === "stat") {
            ...
            if (ch.type === "skill") {
                const targetKey = String(ch.targetKey ?? ch.skill ?? "").trim();
                if (!targetKey) {
                    await this._addBio(run, "Skill reward skipped: missing targetKey/skill.");
                    continue;
                }
                await this._grantSkillToward(run, targetKey, ch.targetLevel, ch.fallback);
            }
            if (ch.type === "item") {
    ...
    async _rollLuckTable(run) {
        ...
        if (!luckTableUuid || luckTableUuid === "your-fixed-luck-roll-table-uuid") {
            return "Lucky streak began, but no Fortune/Luck table UUID is configured.";
        }
    ...
    async _onChoose(index, cardEl) {
        ...
        try {
            ...
            const nextObj =
                ...
                (Array.isArray(data.rewards) ? data.rewards.find(r => r?.next?.tableUuid)?.next : null) ??
                null;
